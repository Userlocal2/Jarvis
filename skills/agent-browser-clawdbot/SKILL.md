...
Use this skill only under all of these constraints:

- Only for public websites or local test environments
- Only for test automation, QA flows, demos, and reproducible browser tasks
- Never use on banking, email, cloud admin, government, healthcare, payroll, or other sensitive sites
- Never log into personal accounts unless the human explicitly approves that exact site and session
- Never save or load browser auth state for real personal or production accounts
- Never exfiltrate cookies, localStorage, session tokens, page contents behind login, or secrets from forms
- Never use `wait --fn`, network mocking/routing, storage inspection, or cookie manipulation on sensitive or authenticated sites
- If a task involves credentials, payments, private dashboards, or personal data, stop and ask first
